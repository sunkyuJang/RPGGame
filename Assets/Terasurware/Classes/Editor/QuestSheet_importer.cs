		...
		foreach (string asset in importedAssets) {
			...
			using (FileStream stream = File.Open (filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)) {
				...
				foreach(string sheetName in sheetNames) {
					...
					cell = row.GetCell(2); p.needItem = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(3); p.needCount = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(4); p.rewardItem = (cell == null ? "" : cell.StringCellValue);
					cell = row.GetCell(5); p.rewardCount = (cell == null ? "" : cell.StringCellValue);
						s.list.Add (p);
					}
					data.sheets.Add(s);
				}
